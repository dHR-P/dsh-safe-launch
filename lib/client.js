...
`;
		...
		function SafeLaunchSettingsCard() {
			...
			if (!st) {
				...
				if (st.launcher && st.launcher.installed) {
			...
			if (cu && cu.available) {
				rows.push(react.createElement("div", { key: "cup",
					style: { border: "1px solid #7f1d1d", borderRadius: "8px", padding: "8px 10px", background: "rgba(229,83,75,.08)", display: "flex", flexDirection: "column", gap: "6px" } },
					react.createElement("div", null,
						react.createElement("b", { style: { color: "#e5534b" } }, "检测到配套更新"),
						`：dsh ${cu.dshLatest} ＋ 安全启动器 v${cu.slTag}`),
					react.createElement("div", { className: "dsh-safelaunch-row" },
						react.createElement("button", {
							className: "dsh-safelaunch-btn dsh-safelaunch-btn-danger",
							disabled: !!busy,
							onClick: async () => {
								if (!window.confirm(`将一起升级：\n· dsh → ${cu.dshLatest}\n· 安全启动器 → ${cu.slTag}\n\n流程：先在隔离环境试运行新组合（不影响当前实例），通过后写入，最后提示重启。继续？`)) return;
								setBusy("一键升级"); setMsg("");
								try {
									const fired = await api("upgrade-pair", { dshVersion: cu.dshLatest, slTag: cu.slTag });
									for (let i = 0; i < 200; i++) {
										await new Promise((r2) => setTimeout(r2, 3000));
										const j = await api("job?id=" + encodeURIComponent(fired.jobId));
										if (j.status === "done") { setMsg("配套升级完成 ✓ 请点击「重启 DSH」应用。"); break; }
										if (j.status === "error") throw new Error(j.error || "任务失败");
										const ls = j.lines || [];
										if (ls.length) setMsg("升级中… " + ls[ls.length - 1].slice(11));
									}
									const v = await api("status"); setSt(v);
								} catch (e) {
									setMsg("一键升级失败：" + (e && e.message ? e.message : e));
								} finally { setBusy(""); }
							},
						}, busy === "一键升级" ? "升级中…" : "一键升级（先试运行）"))));
			}
