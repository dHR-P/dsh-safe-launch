...
async function main() {
	...
	// v0.3 自适应启动命令：优先使用 last-good.json 里验证过的 bootArgs 模板
	// （含 {host}/{port} 占位符）；缺省时回退到当前默认形状。
	let args;
	if (Array.isArray(cfg.bootArgs) && cfg.bootArgs.length > 0 && cfg.bootArgs.every((x) => typeof x === 'string')) {
		const host = cfg.host || '127.0.0.1';
		const p = String(cfg.port);
		args = [`"${cfg.entry}"`, ...cfg.bootArgs.map((s) => s.replaceAll('{host}', host).replaceAll('{port}', p))];
	} else {
		args = [`"${cfg.entry}"`, 'web', '--host', cfg.host || '127.0.0.1', '--port', String(cfg.port), '--no-open'];
	}
