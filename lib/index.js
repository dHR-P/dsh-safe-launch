...
/** Supported host range. min rises ONLY when a dsh release genuinely breaks
 * us; absent max means "newer dsh assumed compatible until proven otherwise"
 * (verified per release via /self-test before we ship a plugin update). */
const DSH_COMPAT = { min: '0.1.1-rc.2', max: null };
...
$p = Start-Process node.exe -ArgumentList @('"' + $cfg.entry + '"', 'web', '--host', $cfg.host, '--port', "$($cfg.port)", '--no-open') -WorkingDirectory $cfg.workdir -WindowStyle Hidden -PassThru
$ok = $false
foreach ($i in 1..90) {
...
if (-not $ok) {
    ...
    if ($snaps.Count -gt 0) {
        ...
        $null = Start-Process node.exe -ArgumentList @('"' + $cfg.entry + '"', 'web', '--host', $cfg.host, '--port', "$($cfg.port)", '--no-open') -WorkingDirectory $cfg.workdir -WindowStyle Hidden -PassThru
        foreach ($i in 1..90) {
            Start-Sleep -Milliseconds 1000
            if (Test-Up) { $ok = $true; break }
        }
...
async function flowSelfTest(emit, versions) {
	...
	for (const v of targets) {
		try {
			...
			const t = await canaryTest({ entry, isoHome: iso, workdir: cfg.workdir, label: `selftest:${v}` });
...
function assessCompat(dshVersion) {
	...
	if (DSH_COMPAT.min && cmpSemver(dshVersion, DSH_COMPAT.min) < 0) {
		return { ok: false, dshVersion, reason: `dsh ${dshVersion} 低于本插件支持的最低版本 ${DSH_COMPAT.min}`, hint: hintBase };
	}
