 ...
 *   POST /dsh-safe-launch/rollback-config   {} -> restore previous last-good config
...
/** Junction mode: profiles\web linked to the real one (read-only reuse; for
 * update canaries where nothing must be written into the profile). */
async function makeIsoJunctionHome(tag) {
	const iso = await makeIsoHomeBase(tag);
	const realProfile = join(DSH_HOME, 'profiles', 'web');
	if (existsSync(realProfile)) {
		await ensureDir(join(iso, 'profiles'));
		await symlink(realProfile, join(iso, 'profiles', 'web'), 'junction');
	}
	return iso;
}
...
async function flowCheckUpdates(emit, { apply = false, includePlugins = false } = {}) {
	...
	if (coreUpdate && apply) {
		...
		const iso = await makeIsoJunctionHome(latest);
		...
		if (t.ok) {
			...
			cfg.plugins = await profileDepsSnapshot(pDir);
...
async function flowUpdatePlugins(emit) {
	...
	const iso = await makeIsoJunctionHome('plugins');
			...
			switch (pathname) {
				...
				case '/dsh-safe-launch/test-candidate': {
					...
					const r = startJob('test-candidate', async (emit) => {
						...
						const iso = await makeIsoJunctionHome(ver);
						...
						cfg2.plugins = await profileDepsSnapshot(profileDirOf(cfg2));
				...
				case '/dsh-safe-launch/rollback-config': {
					...
					json(res, OK({ rolledBack: true, from: picked, version: restored?.dshVersion, note: '下次 start/restart 生效' }));
					return;
				}
