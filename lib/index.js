...
const PKG_VERSION = '0.1.2-alpha.3-v0.5.1';
const DSH_PACKAGE = '@deepseek-ai/dsh';
/** 本插件源码仓库（配对更新检测：GitHub releases/latest）。 */
const PKG_REPO = 'dHR-P/dsh-safe-launch';

/** Compatibility posture (v0.5.2): EXACT version matrix, not default-open.
 * Only versions explicitly marked `supported` below are covered; `untested`
 * (public but not yet verified) and unknown versions are NOT assumed supported.
 * This prevents both "skipped a version → wrongly unsupported" and
 * "default-open → wrongly supported". 每个新的 dsh 公开版本必须逐一实测后
 * 在本表标记 supported/broken，再发插件版本。 */
const DSH_COMPAT = { min: null, max: null, tested: '0.1.2-alpha.3' };

/** dsh 公开版本兼容性矩阵：GitHub tags + npm versions 的完整并集（共 13 个）。
 * status: supported=实测通过 / broken=实测失败(已知不兼容) / untested=公开但尚未实测。
 * 不在本表的版本一律按 unknown 处理（绝不默认宽松）。 */
const DSH_VERSION_MATRIX = [
	{ version: '0.0.1-rc.1', status: 'untested' },
	{ version: '0.0.1-rc.2', status: 'untested' },
	{ version: '0.0.1-rc.5', status: 'untested' },
	{ version: '0.1.0-rc.2', status: 'untested' },
	{ version: '0.1.0-rc.3', status: 'untested' },
	{ version: '0.1.0-rc.6', status: 'untested' },
	{ version: '0.1.0-rc.7', status: 'untested' },
	{ version: '0.1.0-rc.8', status: 'untested' },
	{ version: '0.1.1-rc.1', status: 'untested' },
	{ version: '0.1.1-rc.2', status: 'supported' },
	{ version: '0.1.2-alpha.1', status: 'untested' },
	{ version: '0.1.2-alpha.2', status: 'untested' },
	{ version: '0.1.2-alpha.3', status: 'supported' }
];

/** 查询版本在矩阵中的状态；空/未列入返回 {status:'unknown'}。 */
function versionMatrixStatus(v) {
	const t = String(v || '').trim().replace(/^v/, '');
	if (!t) return { version: '', status: 'unknown' };
	const hit = DSH_VERSION_MATRIX.find((r) => r.version === t);
	return hit ? { version: t, status: hit.status } : { version: t, status: 'unknown' };
}
...
/** 覆盖门禁：目标 dsh 版本是否已被本插件实测覆盖（矩阵中显式标记 supported）。 */
function isCoveredByPlugin(target) {
	const t = String(target || '').trim().replace(/^v/, '');
	if (!t) return false;
	return versionMatrixStatus(t).status === 'supported';
}
...
/** 读取 dsh 在 npm 上的全部 dist-tags（latest/alpha/next...）；失败返回 {}。 */
async function npmViewTags() {
	const r = await runShell(`npm view ${DSH_PACKAGE} dist-tags --json --fetch-timeout=20000 --fetch-retries=1`, { timeoutMs: 40000 });
	if (r.code !== 0) return {};
	try {
...
/** 取 latest 与 alpha 中语义较新者：alpha 是 GitHub 先行发布、只挂 alpha
 * dist-tag 的测试版（不占 latest），与正式版同等提示，使安全启动器能识别
 * 并提示测试版更新。 */
async function npmViewLatest() {
	const tags = await npmViewTags();
	const cands = ['latest', 'alpha']
		.map((k) => (typeof tags[k] === 'string' && /^[\w.\-+]+$/.test(tags[k]) ? tags[k] : ''))
		.filter(Boolean);
	cands.sort((a, b) => cmpSemver(b, a));
	return cands[0] || '';
}
...
async function canaryTest({ entry, isoHome, workdir, label, cfg }) {
	...
	try {
		...
		// 健康检查走插件自身 /ping 端点：dsh >=0.1.2-alpha 给根路径 `/` 加了
		// 一次性 token 认证（无 cookie 返回 401），而插件路由由 webserver 直接
		// 分发、不受 token 保护，且能更精确地证明「dsh 服务 + 本插件均已就绪」。
		const url = `http://127.0.0.1:${port}/dsh-safe-launch/ping`;
...
async function statusPayload(body) {
	...
	// 用插件自身 /ping 端点判断实例在线：根路径 `/` 在 dsh>=0.1.2-alpha 被
	// 一次性 token 认证保护（无 cookie 401），会误判「未运行」。
	const url = `http://${cfg.host}:${cfg.port}/dsh-safe-launch/ping`;
	...
	const payload = {
		...
		versionMatrix: DSH_VERSION_MATRIX,
		currentDshVersionStatus: versionMatrixStatus(cfg.dshVersion).status,
		lastDiagnosis,
		incompatiblePlugins
	};
	if (body && body.network === true) {
		...
		// 识别插件当前是否已声明覆盖 GitHub/npm 上的最新 dsh 版本（tested >= latest）。
		payload.supportsLatest = latest ? isCoveredByPlugin(latest) : null;
...
async function registerRoutes(ctx) {
	// 能力检测：宿主有 webServer 才注册 HTTP API；缺失时降级（仅保留文件级监测/日志/巡检）。
	const ws = ctx.reflect.get('webServer');
	if (!ws) { logLine('宿主缺少 webServer 服务，跳过 HTTP 路由注册（保留文件级监测与日志）'); return; }
	...
	const disposer = ws.register({ kind: 'prefix', path: '/dsh-safe-launch', handler });
...
function registerDormantRoutes(ctx, verdict) {
	try {
		...
		const ws = ctx.reflect.get('webServer');
		if (!ws) { logLine('休眠模式：宿主缺少 webServer，无法注册说明端点'); return; }
		ws.register({ kind: 'prefix', path: '/dsh-safe-launch', handler });
...
function assessCompat(dshVersion) {
	...
	const { version, status } = versionMatrixStatus(dshVersion);
	if (!version) return { ok: true, dshVersion: version, status }; // 无法判定时按可用处理（不因未知而停用）
	// 精确矩阵：只有显式标记 broken 的版本才休眠；supported/untested/unknown 均正常加载，
	// 但 untested/unknown 通过 statusPayload 暴露「未验证」状态，绝不默认视为支持。
	if (status === 'broken') {
		return { ok: false, dshVersion: version, reason: `dsh ${version} 已知与本插件不兼容`, hint: hintBase };
	}
	return { ok: true, dshVersion: version, status };
}
...
async function bootstrapRoutesGuarded(ctx) {
	...
	// 能力检测 + 就绪等待：webServer 可能尚未提供（旧版/加载顺序靠后），
	// 已就绪立即注册；未就绪则监听 internal/service 事件，服务出现后再注册。
	// 无论 HTTP 路由是否可用，文件级监测（巡检/诊断/更新检查）都不受影响。
	const tryRegister = async () => {
		if (!ctx.reflect.get('webServer')) return false;
		try {
	...
	if (!(await tryRegister())) {
		logLine('webServer 服务尚未就绪，待其提供后注册 HTTP 路由（文件级监测已启动）');
		ctx.on('internal/service', (name) => { if (name === 'webServer') tryRegister(); });
	}
}

// 不把 webServer/settings 声明为硬依赖：任意 dsh 版本都能加载本插件；
// apply 内用 ctx.reflect.get 做能力检测，服务缺失时优雅降级为「仅文件级监测」。
const inject = [];
...
async function registerSettingsNamespace(ctx) {
	if (settingsScopeRef) return; // 已注册，避免重复
	const settings = ctx.reflect.get('settings');
	if (!settings) {
		// settings 服务可能晚于本插件提供，监听其出现后再注册；永不出现则跳过（不影响核心监测）。
		ctx.on('internal/service', (name) => { if (name === 'settings') registerSettingsNamespace(ctx); });
		logLine('settings 服务尚未就绪，待其提供后注册设置命名空间');
		return;
	}
	const scope = settings.register(SETTINGS_NS, SETTINGS_SCHEMA);
