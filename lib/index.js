...
async function statusPayload(body) {
	...
	const payload = {
		...
		combinedUpdate,
		slLatest: combinedUpdate ? combinedUpdate.slLatest : null,
	...
	const handler = async (req, res) => {
		...
		try {
			switch (pathname) {
				...
				case '/dsh-safe-launch/upgrade-pair': {
					const dshVer = typeof payload.dshVersion === 'string' ? payload.dshVersion.trim().replace(/^v/, '') : '';
					const slTagRaw = typeof payload.slTag === 'string' ? payload.slTag.trim().replace(/^v/, '') : '';
					if (!/^[\w.-]+$/.test(dshVer) || !/^[\w.-]+$/.test(slTagRaw)) { json(res, FAIL(BAD_REQUEST)); return; }
					if (dshPrefixOf(slTagRaw) !== dshVer && payload.force !== true) {
						json(res, FAIL({ code: 'not-paired', message: `安全启动器 ${slTagRaw} 的适配前缀(${dshPrefixOf(slTagRaw)}) 与目标 dsh ${dshVer} 不一致` }), 409);
						return;
					}
					const r = startJob('upgrade-pair', async (emit) => {
						const cfg2 = await ensureConfig();
						const rootDir = dirname(dirname(fileURLToPath(import.meta.url)));
						const prevRef = (await runDirect('git', ['-C', rootDir, 'rev-parse', '--short', 'HEAD'], { timeoutMs: 30000 })).stdout.trim();
						emit(`更新安全启动器自身 -> ${slTagRaw}（git fetch + checkout）...`);
						let g = await runDirect('git', ['-C', rootDir, 'fetch', '--tags', '--force'], { timeoutMs: 120000 });
						if (g.code !== 0) throw new Error('git fetch 失败: ' + (g.stderr || g.stdout));
						g = await runDirect('git', ['-C', rootDir, 'checkout', slTagRaw], { timeoutMs: 60000 });
						if (g.code !== 0) throw new Error(`checkout ${slTagRaw} 失败: ` + (g.stderr || g.stdout));
						await appendAudit({ kind: 'self-updated', summary: slTagRaw });
						emit(`下载并试运行新 dsh ${dshVer}...`);
						const entry = await installRuntime(dshVer, emit);
						const iso = await makeIsoJunctionHome(dshVer, cfg2);
						const tt = await canaryTest({ entry, isoHome: iso, workdir: cfg2.workdir, label: 'pair:' + dshVer, cfg: cfg2 });
						if (!tt.ok) {
							await runDirect('git', ['-C', rootDir, 'checkout', prevRef]).catch(() => {});
							await appendNotice(`dsh ${dshVer} 试运行未通过，安全启动器已回退到 ${prevRef}。原因: ${tt.detail}`);
							throw new Error(`试运行未通过: ${tt.detail}`);
						}
						cfg2.entry = entry;
						cfg2.dshVersion = dshVer;
						cfg2.source = 'pair-promoted';
						await withManifestSnapshot(cfg2, 'pair-promoted');
						await saveLastGood(cfg2, 'pair-promoted', `dsh=${dshVer};plugin=${slTagRaw}`);
						await appendNotice(`配套升级完成：dsh ${dshVer} ＋ 安全启动器 ${slTagRaw}。重启 DSH 后生效。`);
						return { promoted: true, version: dshVer, pluginTag: slTagRaw, requiresRestart: true };
					});
					if (r.error) { json(res, FAIL(r.error), 409); return; }
					json(res, OK({ jobId: r.jobId }));
					return;
	...
	ensureDir(SL_DIR).then(async () => {
		...
		if (!dormantVerdict) {
			...
			waitForSelfHttp(120000)
				.then((up) => { if (up) return startupUpdateNotice(); logLine('网页未就绪，跳过启动更新检查'); })
